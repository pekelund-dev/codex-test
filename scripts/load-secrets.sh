#!/usr/bin/env bash
#
# Load secrets from JSON credential files
...
if [[ "${BASH_SOURCE[0]}" == "${0}" ]]; then
  ...
  echo "   Run: source scripts/load-secrets.sh" >&2
  exit 1
fi
...
load_google_oauth_credentials() {
  ...
  # Parse OAuth credentials JSON using Python
...
# Try different possible keys for OAuth client structure
candidate = None
...
load_gcs_credentials() {
  ...
  if [[ ! -f "$GCS_CREDENTIALS_FILE" ]]; then
    echo "⚠️  GCS_CREDENTIALS_FILE points to '$GCS_CREDENTIALS_FILE' but the file was not found." >&2
    return
  fi

  if [[ "${GCS_CREDENTIALS:-}" != "file:${GCS_CREDENTIALS_FILE}" ]]; then
    export GCS_CREDENTIALS="file:${GCS_CREDENTIALS_FILE}"
  fi

  echo "✅ Loaded GCS credentials from ${GCS_CREDENTIALS_FILE}"
}
