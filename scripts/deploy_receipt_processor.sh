...
GOOGLE_AI_GEMINI_MODEL="${GOOGLE_AI_GEMINI_MODEL:-gemini-2.0-flash}"
RECEIPT_FIRESTORE_COLLECTION="${RECEIPT_FIRESTORE_COLLECTION:-receiptExtractions}"
SPRING_PROFILES_ACTIVE="${SPRING_PROFILES_ACTIVE:-prod}"

if [[ -z "${AI_STUDIO_API_KEY}" ]]; then
  echo "AI_STUDIO_API_KEY must be set to a Google AI Studio Gemini API key or Secret Manager reference before deployment." >&2
  exit 1
fi
...
append_env_var "GOOGLE_AI_GEMINI_MODEL" "$GOOGLE_AI_GEMINI_MODEL"
