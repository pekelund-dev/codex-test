...
5. **Configure the Spring Boot app**
   - Keep downloaded JSON keys outside the repository (for example `~/.config/pklnd/firestore.json`). Point the helper at those files so the environment variables are populated without copying secrets into your shell history:
...
3. **Create a bucket**
   ...
   - Provide a globally unique name (for example `pklnd-receipts`).
...
4. **Create a dedicated service account**
   ...
   - Name it something like `pklnd-receipts-uploader` and grant **Storage Object Admin**.
...
6. **Configure environment variables**
   ...
   export GCS_BUCKET=pklnd-receipts           # Replace with your bucket name
