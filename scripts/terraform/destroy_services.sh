...
if [[ -z "${PROJECT_ID}" ]]; then
  echo "PROJECT_ID must be set or configured in gcloud before destroying services." >&2
  exit 1
fi

bucket_name=${BUCKET_NAME:-}
web_sa=${WEB_SERVICE_ACCOUNT_EMAIL:-}
receipt_sa=${RECEIPT_SERVICE_ACCOUNT_EMAIL:-}
...
if [[ -d "${INFRA_DIR}" ]]; then
  ...
  secret_from_tf=$(tf_output_raw app_secret_name)
  ...
  APP_SECRET_NAME=${secret_from_tf:-${APP_SECRET_NAME}}
fi

bucket_name=${bucket_name:-"pklnd-receipts-${PROJECT_ID}"}
web_sa=${web_sa:-"cloud-run-runtime@${PROJECT_ID}.iam.gserviceaccount.com"}
receipt_sa=${receipt_sa:-"receipt-processor@${PROJECT_ID}.iam.gserviceaccount.com"}

web_image="${REGION}-docker.pkg.dev/${PROJECT_ID}/${web_repo}/${WEB_SERVICE_NAME}:placeholder"
receipt_image="${REGION}-docker.pkg.dev/${PROJECT_ID}/${receipt_repo}/${RECEIPT_SERVICE_NAME}:placeholder"
...
if gcloud secrets describe "${APP_SECRET_NAME}" --project "${PROJECT_ID}" >/dev/null 2>&1; then
  ...
  echo "Secret ${APP_SECRET_NAME} not found in project ${PROJECT_ID}. Proceeding with empty credentials for destroy." >&2
fi
