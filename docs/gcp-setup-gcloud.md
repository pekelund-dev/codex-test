...
1. **Create (or select) your project**
    ...
    ```bash
    gcloud projects create pklnd-app --set-as-default
...
3. **Create a service account for the Spring Boot app**
    ...
    ```bash
    FIRESTORE_SA=pklnd-firestore@$(gcloud config get-value project).iam.gserviceaccount.com
    gcloud iam service-accounts create pklnd-firestore \
      --display-name="pklnd Firestore client"
...
5. **Export the application environment variables**
    ...
    Keep any downloaded keys or OAuth credentials outside of the repository (for example under `~/.config/pklnd/`). Point environment variables at the files and source the helper so every shell inherits the derived values:
...
1. **Create the receipts bucket**
    ...
    ```bash
    BUCKET=gs://pklnd-receipts-$(gcloud config get-value project)
...
2. **Create a service account dedicated to uploads**
    ...
    ```bash
    UPLOAD_SA=pklnd-receipts@$(gcloud config get-value project).iam.gserviceaccount.com
    gcloud iam service-accounts create pklnd-receipts \
      --display-name="pklnd receipts uploader"
...
2. **Create the receipt processor service account** and grant required roles:
    ...
    BUCKET=gs://pklnd-receipts-${PROJECT_ID}  # Replace if you use a custom bucket
...
1. **Grant the web service account invocation rights** so uploads can trigger parsing. Replace the example identity if your Cloud Run web app uses a different service account.
    ...
    ```bash
    WEB_APP_SA=pklnd-run-sa@${PROJECT_ID}.iam.gserviceaccount.com
