...
/**
 * Handles the business logic for parsing receipt files that were uploaded to Cloud
 * Storage.
 */
@Service
public class ReceiptProcessingService {
    ...
    public ReceiptProcessingService(Storage storage, ReceiptExtractionRepository repository,
        ReceiptDataExtractor extractor) {
        this.storage = storage;
        this.repository = repository;
        this.extractor = extractor;
        LOGGER.info("Constructing ReceiptProcessingService with storage {}, repository {}, extractor instance id {}",
            storage.getClass().getName(), repository.getClass().getName(), System.identityHashCode(extractor));
    }

    public void process(ReceiptProcessingMessage message) {
        LOGGER.info("ReceiptProcessingService invoked with message {}", message);
        if (message == null) {
            LOGGER.warn("Received null Pub/Sub message payload");
            return;
        }
        ...
        LOGGER.info("ReceiptProcessingService processing object gs://{}/{} with extractor instance id {}", bucket, objectName,
            System.identityHashCode(extractor));

        if (!StringUtils.hasText(bucket) || !StringUtils.hasText(objectName)) {
            LOGGER.warn("Message missing bucket ({}) or object name ({})", bucket, objectName);
            return;
        }
        ...
        try {
            ...
            if (!isPdf(blob)) {
                String statusMessage = "Only PDF receipts are processed";
                repository.markStatus(bucket, objectName, owner, ReceiptProcessingStatus.SKIPPED, statusMessage);
                blob = updateProcessingMetadata(blob, ReceiptProcessingStatus.SKIPPED, statusMessage);
                return;
            }
            ...
            int rawResponseLength = extractionResult.rawResponse() != null
                ...
                : 0;
            LOGGER.info("ReceiptProcessingService extracted {} top-level fields and {} items (raw response length {} characters) for gs://{}/{}",
            ...
            LOGGER.info("ReceiptProcessingService successfully completed extraction for gs://{}/{}", bucket, objectName);
